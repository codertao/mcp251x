...
static void mcp251x_dump_regs(struct spi_device *spi) {
	u8 i, j;
	u8 line[16];
	struct mcp251x_priv *priv = spi_get_drvdata(spi);
	...
	for(i=0; i<128; i+=16) {
		priv->spi_tx_buf[0] = INSTRUCTION_READ;
		priv->spi_tx_buf[1] = i;
		mcp251x_spi_trans(spi, 10);
		for(j=0; j<8; j++) line[j] = priv->spi_rx_buf[2+j];

		priv->spi_tx_buf[0] = INSTRUCTION_READ;
		priv->spi_tx_buf[1] = i+8;
		mcp251x_spi_trans(spi, 10);
		for(j=0; j<8; j++) line[j+8] = priv->spi_rx_buf[2+j];
		

		dev_info(&spi->dev, "DUMP 0x%02x: %02x %02x %02x %02x  %02x %02x %02x %02x    %02x %02x %02x %02x  %02x %02x %02x %02x", i,
			line[0],
			line[1],
			line[2],
			line[3],
...
static int mcp251x_set_normal_mode(struct spi_device *spi)
{
	...
	priv->can.state = CAN_STATE_ERROR_ACTIVE;
	dev_info(&spi->dev, "in normal mode\n");
...
MODULE_AUTHOR("Chris Elston <celston@katalix.com>, "
	      "Christian Pellegrin <chripell@evolware.org>, Matt <matt@codertao.com>");
